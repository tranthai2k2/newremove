...
class ImageTagManager:
    ...
    def toggle_tag(self, tag_key, tag_value):
        """
        Logic mới:
        - Nếu tìm thấy key trong prompt -> Kích hoạt chế độ XÓA.
        - Khi XÓA: Cố gắng xóa cả '(( fat man ))' lẫn 'fat man'.
        - Sau đó dọn dẹp dấu phẩy thừa.
        """
        content = self.text_area.get(1.0, tk.END).strip()
        key_lc = tag_key.lower()

        # Kiểm tra xem từ khóa có tồn tại không (chỉ cần chứa key là coi như tồn tại)
        if key_lc in content.lower():
            # --- XÓA ---
            
            # 1. Xóa TAG_VALUE trước (ưu tiên dạng đầy đủ có ngoặc đặc biệt)
            # Dùng re.escape để Python hiểu đúng các ký tự (( ))
            escaped_value = re.escape(tag_value)
            content = re.sub(rf"(?i){escaped_value}", "", content)

            # 2. Xóa TAG_KEY (dạng text thuần, ví dụ 'fat man')
            # Dùng \b để đảm bảo chỉ xóa đúng từ đó (tránh xóa 'man' trong 'woman' nếu key ngắn)
            escaped_key = re.escape(tag_key)
            content = re.sub(rf"(?i)\b{escaped_key}\b", "", content)

            # 3. Dọn dẹp dấu phẩy và khoảng trắng thừa (Clean up)
            # Thay thế nhiều dấu phẩy liên tiếp thành 1
            content = re.sub(r",\s*,", ", ", content)
            # Xóa dấu phẩy ở đầu chuỗi
