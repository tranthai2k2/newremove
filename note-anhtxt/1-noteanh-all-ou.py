...
class ImageCompareApp:
    def __init__(self, root):
        ...
        self.addfaceless_content = []
        self.enable_prompt_compare = tk.BooleanVar(value=False)  # ← MỚI
        self.canvas2_list = []
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    self.enable_prompt_compare.set(config.get('enable_prompt_compare', False))
            except:
                pass
    ...
    def save_config(self):
        config = {
            ...
            'enable_prompt_compare': self.enable_prompt_compare.get()
        }
    ...
    def setup_ui(self):
        ...
        # Row 0: Folder 1 + TXT Mode + Compare Mode
        ...
        # ← MỚI: Compare Mode
        compare_frame = ttk.LabelFrame(folder_frame, text="So sánh Prompt", padding="5")
        compare_frame.grid(row=0, column=4, padx=10)
        ttk.Checkbutton(
            compare_frame, 
            text="Bật so sánh\n(all_tags → addfaceless)",
            variable=self.enable_prompt_compare,
        ...
        control_frame.grid(row=2, column=1, pady=20)
        ...
        ttk.Button(control_frame, text="Export Excel", command=self.export_excel).pack(side=tk.LEFT, padx=5)

        # TXT File Selection
        self.txt_select_frame = ttk.LabelFrame(
            folder_frame, text="Chọn File TXT từ out_tags folder", padding="10"
        )
        self.txt_select_frame.grid(row=3, column=0, columnspan=5, pady=10, sticky="ew")
    ...
    def on_compare_mode_change(self):
        """Callback khi thay đổi chế độ so sánh"""
        if self.enable_prompt_compare.get():
            self.status_label.config(
                text="✓ Đã bật so sánh prompt (all_tags → addfaceless)",
                foreground='green'
            )
        else:
            self.status_label.config(
                text="So sánh prompt đã tắt",
                foreground='gray'
            )
        self.save_config()
    ...
    def load_txt_content(self, folder1):
        """Load TXT content với option so sánh prompt"""
        txt_mode = self.txt_mode.get()
        txt_file_choice = self.txt_file_choice.get()
        ...
        if txt_mode == 'all_tags':
            ...
            # Load addfaceless.txt nếu bật chế độ so sánh
            if self.enable_prompt_compare.get():
                addfaceless_path = os.path.join(out_tags_folder, 'addfaceless.txt')
                if os.path.exists(addfaceless_path):
                    with open(addfaceless_path, 'r', encoding='utf-8') as f:
                        self.addfaceless_content = [line.strip() for line in f.readlines()]
                    status_msg = f"Mode: {txt_filename} ({len(self.all_tags_content)} dòng) + addfaceless ({len(self.addfaceless_content)} dòng)"
                else:
                    messagebox.showwarning("Cảnh báo", "Không tìm thấy addfaceless.txt, bỏ qua so sánh")
                    self.addfaceless_content = []
                    status_msg = f"Mode: {txt_filename} ({len(self.all_tags_content)} dòng) - Không có addfaceless"
            else:
                self.addfaceless_content = []
    ...
    def load_data(self):
        ...
        for idx, img1 in enumerate(folder1_images):
            ...
            # Load prompt before/after nếu bật so sánh
            prompt_before = ""
            prompt_after = ""

            if self.txt_mode.get() == 'all_tags' and self.enable_prompt_compare.get():
                if idx < len(self.all_tags_content):
                    prompt_before = self.all_tags_content[idx]

                if idx < len(self.addfaceless_content):
                    prompt_after = self.addfaceless_content[idx]
    ...
    def export_excel(self):
        ...
        # Tạo headers tùy theo chế độ so sánh
        headers = ["STT", "Image 1"]
        for i in range(num_img2):
            headers.append(f"Image 2-{i+1}")

        if self.enable_prompt_compare.get() and self.addfaceless_content:
            headers.extend([
                "Prompt (Display)",
                "Prompt Before (all_tags)",
                "Prompt After (addfaceless)",
                "Notes"
            ])
        else:
        ...
        prompt_col_idx = 67 + num_img2
        ws.column_dimensions[chr(prompt_col_idx)].width = 50

        if self.enable_prompt_compare.get() and self.addfaceless_content:
            ws.column_dimensions[chr(prompt_col_idx + 1)].width = 50
            ws.column_dimensions[chr(prompt_col_idx + 2)].width = 50
            ws.column_dimensions[chr(prompt_col_idx + 3)].width = 50
        else:
            ws.column_dimensions[chr(prompt_col_idx + 1)].width = 50
        ...
        for idx, data in enumerate(self.data_list, start=4):
            ...
            if self.enable_prompt_compare.get() and self.addfaceless_content:
                ...
                before_col = chr(68 + num_img2)
                after_col = chr(69 + num_img2)
                note_col = chr(70 + num_img2)

                ws[f"{prompt_col}{idx}"] = data['prompt']
                ws[f"{before_col}{idx}"] = data.get('prompt_before', '')
                ws[f"{after_col}{idx}"] = data.get('prompt_after', '')
        ...
        ws_info['A9'] = "Prompt Comparison:"
        if self.enable_prompt_compare.get():
            ws_info['B9'] = f"Enabled - Before: {self.txt_file_choice.get()}.txt | After: addfaceless.txt"
        else:
            ws_info['B9'] = "Disabled"
