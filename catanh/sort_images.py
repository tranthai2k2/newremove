...
Hỗ trợ: Kéo thả, Preview grid, Rename file + TXT metadata
Chạy: python sort_images.py
"""
...
def create_metadata_txt(filepath: str, index: int, total: int, original_name: str = ''):
    ...
    Chứa: số thứ tự, tên gốc, ngày tạo, kích thước, etc
    """
    try:
...
def apply_rename_files(folder_path: str, old_names: list, new_names: list):
    """
    ⚠️ RENAME FILE THỰC TẾ + TẠO TXT METADATA
    Đổi tên file + tạo file TXT tương ứng
    
    Args:
        folder_path: Đường dẫn folder
        old_names: Danh sách tên cũ (original order)
        new_names: Danh sách tên mới (new order)
    """
    ...
    try:
        ...
        # BƯỚC 1: Rename tất cả thành tên tạm (backup)
        temp_mapping = {}
        ...
        # BƯỚC 2: Rename từ tên tạm sang tên mới + Tạo TXT
...
<head>
    ...
    <title>🖼️ Sắp xếp Ảnh - Grid Preview</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🖼️ Sắp xếp Ảnh</h1>
            <p>Kéo thả để sắp xếp | Xem trước | Apply (tạo TXT)</p>
        </div>
        ...
        <div class="toolbar">
            ...
            <button class="btn danger" onclick="showApplyModal()" id="applyBtn" style="display:none;">⚡ Apply + TXT</button>
        </div>
        ...
        <div class="footer">
            <p>⚡ Kéo thả | Preview | Export | Apply (tạo TXT metadata)</p>
        </div>
    </div>
    ...
    <div id="applyModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><strong>Bạn sắp thực hiện thao tác không thể hoàn tác!</strong></p>
                <p>Hệ thống sẽ:</p>
                ...
                <p style="color:#c33; margin-top:15px;">🔐 Hãy chắc chắn bạn đã backup trước!</p>
            </div>
            <div class="modal-footer">
                <button class="btn secondary" onclick="closeApplyModal()">❌ Hủy</button>
                <button class="btn danger" onclick="confirmApplyRename()">✅ Đồng ý Apply</button>
            </div>
        </div>
    </div>
        ...
        <div class="preview-content">
            ...
            <div class="preview-header">
                ...
                <div class="preview-info">
                    <p>📦 Kích thước: <span id="previewSize"></span></p>
                    <p>🔢 Vị trí: <span id="previewIndex"></span></p>
                </div>
            </div>
    ...
    if (draggedItem && draggedFromIndex !== -1 && draggedFromIndex !== toIndex) {
        // Lấy phần tử bị kéo ra
        const moved = currentImages.splice(draggedFromIndex, 1)[0];
        
        // Nếu kéo từ dưới lên trên, sau khi splice index sẽ thay đổi
    ...
    draggedFromIndex = -1;
        ...
        async function previewImage(index) {
            ...
            if (imageCache[img.name]) {
                ...
                document.getElementById('previewSize').textContent = img.size_mb + ' MB';
                document.getElementById('previewIndex').textContent = (index + 1) + ' / ' + currentImages.length;
                document.getElementById('previewImage').src = imageCache[img.name];
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('previewSize').textContent = img.size_mb + ' MB';
                    document.getElementById('previewIndex').textContent = (index + 1) + ' / ' + currentImages.length;
                    document.getElementById('previewImage').src = data.data;
                    document.getElementById('previewModal').classList.add('show');
                } else {
        ...
        async function exportList() {
            ...
            try {
                ...
                if (data.error) {
                    ...
                    showSuccess('Đã export ' + data.count + ' ảnh thành công!');
                }
            } catch (e) {
        ...
        async function confirmApplyRename() {
            ...
            try {
                ...
                document.getElementById('applyBtn').textContent = '⚡ Apply + TXT';
            }
        }
...
def start_server(port=5000):
    ...
    print("\n" + "="*60)
    print("🖼️  Sắp xếp Ảnh Web UI - Với TXT Metadata")
    print("="*60)
